...
bool StartConnection(P_EIDCLIENT_CONNECTION_HANDLE hConnection, const char* const url, const char* const SessionIdentifier, const char *const PathSecurityParameters)
{
    ...
    if(0x00 != PathSecurityParameters)
    {
		...
		EID_CLIENT_CONNECTION_ERROR rVal = eIDClientConnectionStartHttp(hConnection, strUrl.c_str(), strSessionIdentifier.c_str(), strPSKKey.c_str(), HttpHeaderInclusion::DontGetHttpHeader);
    ...
    {
        strPSKKey.assign("");
		EID_CLIENT_CONNECTION_ERROR rVal = eIDClientConnectionStartHttp(hConnection, strUrl.c_str(), NULL, NULL, HttpHeaderInclusion::DontGetHttpHeader);
	...
	const nPAeIdProtocolStateCallback_t fnCurrentStateCallback)
{
	...
	if( paraMap.m_transactionURL && strlen(paraMap.m_transactionURL) > 0 ) {
		...
		err = eIDClientConnectionStartHttp(&hConnection, strURL.c_str(), NULL, NULL, HttpHeaderInclusion::DontGetHttpHeader);
